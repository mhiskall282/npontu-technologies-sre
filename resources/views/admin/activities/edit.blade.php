            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="priority" class="block w-full rounded-lg border border-gray-300 text-sm focus:ring-[#1B6B3A] focus:border-[#1B6B3A]">
                        <option value="low" {{ old('priority', $activity->priority) === 'low' ? 'selected' : '' }}>P4 — Low</option>
                        <option value="medium" {{ old('priority', $activity->priority) === 'medium' ? 'selected' : '' }}>P3 — Medium</option>
                        <option value="high" {{ old('priority', $activity->priority) === 'high' ? 'selected' : '' }}>P2 — High</option>
                        <option value="critical" {{ old('priority', $activity->priority) === 'critical' ? 'selected' : '' }}>P1 — Critical</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">SLA Target Time</label>
                    <input type="text" name="sla_time" value="{{ old('sla_time', $activity->sla_time) }}" placeholder="e.g. 09:00 GMT" class="block w-full rounded-lg border border-gray-300 text-sm focus:ring-[#1B6B3A] focus:border-[#1B6B3A]">
                </div>
            </div>
            <div class="flex items-center gap-6">
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="is_pinned" value="1" id="is_pinned" class="rounded border-gray-300 text-amber-500 focus:ring-amber-500" {{ old('is_pinned', $activity->is_pinned) ? 'checked' : '' }}>
