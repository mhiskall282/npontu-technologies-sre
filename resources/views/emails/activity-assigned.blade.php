...
<head>
    ...
    <title>SRE Task Assignment</title>
</head>
                                ...
                                <tr>
                                    <td>
                                        <div style="font-size: 11px; font-weight: 800; color: #F5C518; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 4px;">
                                            OPSORA SRE PLATFORM — TASK NOTIFICATION
                                        </div>
                    ...
                    <tr>
                        <td style="padding: 30px;">
                            ...
                            <p style="font-size: 14px; line-height: 1.6; color: #4b5563; margin-top: 0; margin-bottom: 20px;">
                                You have been assigned an operational task by <strong>{{ $assigner?->name ?? 'SRE Lead' }}</strong>:
                            </p>

                            {{-- Task Details Card --}}
                            <div style="background-color: #f9fafb; border-left: 4px solid #1B6B3A; border-radius: 8px; padding: 18px 20px; margin-bottom: 24px;">
                                <div style="font-size: 14px; font-weight: 600; color: #111827; margin-bottom: 8px; line-height: 1.5;">
                                    {{ $activity->description }}
                                </div>
                                <div style="font-size: 12px; color: #6b7280; font-family: monospace;">
                                    Status: <strong style="color: #1B6B3A; text-transform: uppercase;">{{ str_replace('_', ' ', $activity->status) }}</strong>
                                    &bull; Date: {{ $activity->activity_date?->format('Y-m-d') }}
